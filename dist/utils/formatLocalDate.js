...
exports.formatLocalDate = void 0;
// /**
 ...
 * @returns {string} - La date formatée au format local.
 */
var formatLocalDate = exports.formatLocalDate = function formatLocalDate(isoDate) {
  var options = {
    year: "2-digit",
    month: "2-digit",
    day: "2-digit"
  ...
  switch (locale) {
    case "fr-FR":
      // Locale française
      return new Date(isoDate).toLocaleDateString(locale, options).replace(/-/g, "/");
    // Remplace les tirets par des slashs pour la locale française
    default:
      return new Date(isoDate).toLocaleDateString(locale, options).replace(/\//g, "-");
    // Remplace les slashs par des tirets pour les autres locales, y compris en US
  }
};